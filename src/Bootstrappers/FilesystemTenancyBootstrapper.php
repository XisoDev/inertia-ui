...
namespace Xiso\InertiaUI\Bootstrappers;
...
class FilesystemTenancyBootstrapper implements TenancyBootstrapper
{
    ...
    public function bootstrap(Tenant $tenant)
    {
        ...
        foreach ($this->app['config']['tenancy.filesystem.disks'] as $disk) {
            ...
            $finalPrefixUrl = str_replace(
                ['%storage_path%', '%tenant%'],
                [storage_path(), $tenant->getTenantKey()],
                $this->app['config']["tenancy.filesystem.root_override_url.{$disk}"] ?? '',
            );

            if (! $finalPrefixUrl) {
                $finalPrefixUrl = $originalUrl
                    ? rtrim($originalUrl, '/') . '/'. $suffix
                    : $suffix;
            }

            $this->app['config']["filesystems.disks.{$disk}.root"] = $finalPrefix;
            $this->app['config']["filesystems.disks.{$disk}.url"] = $finalPrefixUrl;
        }
    }
