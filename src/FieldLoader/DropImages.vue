<script>
export default {
    name: "DropImages", // vue component name
    data() {
        return {
            error: "",
            files: [],
    ...
    async mounted() {
        if(this.uploaded.length > 0){
            for(let index in this.uploaded){
                this.files.push(await this.getFileFromUrl(this.uploaded[index]));
                this.Imgs.push(this.uploaded[index]);
            }
        }
    },
    methods: {
        ...
        deleteImg(index) {
            ...
            this.$emit("changed", this.files, this.uploaded);
    ...
    >
        ...
        <div class="imgsPreview" v-show="Imgs.length > 0">
            ...
            <div class="imageHolder" v-for="(img, i) in Imgs" :key="i">
                ...
                <div class="plus" @click="append" v-if="++i === Imgs.length && Imgs.length < max">+</div>
            </div>
        </div>
    </div>
</template>
